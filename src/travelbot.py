...
from src.logger import log_response
...
# --- Feedback Capture (CLI only) ---
def collect_user_feedback_cli():
    print("\n📊 Please rate the response below (1 = poor, 5 = excellent):")
    try:
        feedback = {
            "accuracy": int(input("   🔍 Accuracy (1–5): ")),
            "helpfulness": int(input("   🤝 Helpfulness (1–5): ")),
            "clarity": int(input("   🗣️ Clarity (1–5): ")),
            "citation": int(input("   📚 Citation quality (1–5): ")),
            "overall": int(input("   🌟 Overall rating (1–5): "))
        }
        return feedback
    except ValueError:
...
def log_feedback(prompt, response, feedback):
    if feedback:
        ...
        with open("logs/response_feedback.jsonl", "a") as f:
            f.write(json.dumps({
...
def hybrid_response(query, llm, retriever, feedback_score=None, mode="ui"):
    ...
    # Step 5: Generate response summaries
    responses = []
    ...
    # Prepare source list for logging
    source_list = [doc.metadata.get("source", "") for doc in retrieved_docs]

    # 📝 Log feedback only if provided (UI or CLI with collected feedback)
    if feedback_score is not None:
        from src.logger import log_response  # or keep your internal function if not separated
        log_response(
            prompt=query,
    ...
    return final_response, source_list
...
def run_cli(llm, retriever):
    ...
    while True:
        ...
        if query.lower() in ["exit", "quit"]:
            break
        response, sources = hybrid_response(query, llm, retriever, mode="cli")
        feedback = collect_user_feedback_cli()
        if feedback:
            from src.logger import log_response
            log_response(query, response, sources, feedback)
...
if __name__ == "__main__":
    ...
    import atexit
    atexit.register(lambda: getattr(llm, '__del__', lambda: None)())
